...
#include <stdbool.h>
#include <pthread.h>
#define NO_OF_TELECAST 4
#define NO_OF_REPORTS 99
#define NO_OF_EDITORS 2
#define REPORTERS_PER_EDITOR 2
#define NO_OF_UPDATES 99
...
bool check_valid(int topic_id,int valid_bit,int** tele){
	int i=0;
			//printf("in check_valid topic_id=%d, valid_bit = %d\n", tele[topic_id][i], valid_bit);
...
int main(int argc, char *argv[])
{
	...
	int index=NO_OF_EDITORS;
	for(i=0;i<NO_OF_EDITORS;i++){
	...
	if (rank < NO_OF_EDITORS) 
	{
		...
		{	
			...
			//
			int v=atoi(argv[rank+1]);
			
			while(NumProcessesComplete < v)
			{	//printf("\ncheck here editor %d\n", rank);
			...
			if(rank==0){
				for(i=0;i<NO_OF_TOPICS;i++){
					if(strcmp(topics[i],"-1")!=0){
						...
						else printf("Invalid news found \n");	
					}
